...
Let's process the model output to see how prevalence + incidence vary over time.

```{r process_outputs, exercise =TRUE}
...
full_output<- bind_rows(output, output_control)

# calculate time in years
full_output$time_year <- full_output$timestep / year
vaccinetime <- 1   # the year the vaccine is introduced into the population
...
incidence_plot<- function(full_output){
  ...
  geom_vline(xintercept = vaccinetime, linetype= 'dotted', alpha = 10, show.legend = TRUE)+
    theme_bw()+
...
Now let's plot parasite prevalence:

```{r plot_pfpr, exercise = TRUE}
...
ggplot(data= full_output, mapping = aes(x= time_year, y= pfpr, color = scenario, fill= scenario))+
  ...
  #geom_smooth(method = 'loess', span= 0.25) +
  ylim(c(0, 1))+
  geom_vline(xintercept = vaccinetime, linetype= 'dashed', alpha = 10, show.legend = TRUE)+
  theme_bw()+
  scale_color_manual(values= c("#E69F00", "#56B4E9"))+
  scale_fill_manual(values = c("#E69F00", "#56B4E9")) +
  labs(x = "Time (years)", 
...
output$scenario<- 'vaccine' # make an identifying variable for vaccine scenario
full_output<- bind_rows(output, output_control) # bind control and vaccine outputs together
