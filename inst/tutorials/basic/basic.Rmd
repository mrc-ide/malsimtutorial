---
title: "malariasimulation: basic tutorial"
...
![](Images/basic_malsim_process.pdf)

<img src="Images/testing.png" style="display:block; margin: 0 auto; border-width: 0; width: 100%"/>
...
Try running it to take to see what it generates!
...
To understand what each parameter does in the model, you can go to the [help page](https://mrc-ide.github.io/malariasimulation/reference/get_parameters.html) (use `?get_parameters()` when you are in R to see the same information).


Please don't try to understand all of these right now! Instead, try answering the questions below for now, and if there are any parameters you'd like to know more about, ask a demonstrator.

```{r parameters_questions, exercise=TRUE}
quiz(
  question("How long does an untreated asymptomatic infection (A) last for (in days)?",
         answer("1"),
         answer("5"),
         answer("110"),
         answer("195", correct = TRUE)),
  question("What is the default mosquito species?",
         answer("An. arabiensis"),
         answer("An. funestus"),
         answer("An. gambiae", correct = TRUE),
         answer("An. stephensi")),
  question("What is the default adult mosquito mortality rate?",
         answer("0.112"),
         answer("0.132", correct = TRUE),
         answer("0.249"),
         answer("0.643")),
  question("What is the default human population size?",
         answer("10"),
         answer("100", correct = TRUE),
         answer("1000"),
         answer("10,000")),
  question("What is the default mosquito population size?",
         answer("10"),
         answer("100"),
         answer("1000", correct = TRUE),
         answer("10,000")),
  question("What is the default initial susceptible human state proportion?",
         answer("0.42"),
         answer("0.0007", correct = TRUE),
         answer("0.44"),
         answer("0.13")),
...
Any of these parameters can be adjusted within the `get_parameters()` function using the overrides argument:

```{r parameters_override, exercise=TRUE}
get_parameters(overrides = list(human_population = 1000))
```

Most of the parameters have been carefully calibrated so will not need adjusting manually. An exception in the human population size!
...
We generally store a parameter set in an object as so:

```{r parameters_store, exercise=TRUE}
parameters <- get_parameters()
```

By doing this, we can also adjust individual parameters manually outside of the `get_parameters()` function.
...
`malariasimulation` also contains various helper functions and in-built parameters to help you design your parameter set. We will not cover these in detail here, but they include:

Demography (`set_demography()`), mosquito species and proportions (`set_species()`, with in built species specific parameter sets: An. arabiensis (`arab_params()`), An. funestus (`fun_params()`), An. gambiae (`gamb_params()`) and An. stephensi (`steph_params()`)), drug interventions including MDA and SMC (`set_drugs()`, `set_clinical_treatment()`, `set_mda()`, `set_smc()`), bednet and IRS vector control (`set_bednets()`, `set_spraying()`), and pre-erythrocytic and transmission blocking vaccines (`set_mass_pev()`, `set_pev_epi()`, `set_tbv()`). Please see the package vignettes for more details on how to use these, or ask a demonstrator.
...
Do you remember how we saw default parameters for the initial human state proportions and human immunity levels? These may not be applicable to the scenario you are trying to simulate (e.g., particularly if you are starting with no malaria immunity in a high-transmission malaria-endemic area!).
...
Again, there's a lot of information that's being outputted! To understand these outputs, we can view the [help page](https://mrc-ide.github.io/malariasimulation/reference/run_simulation.html) (or use `?get_parameters()` when you are in R to see the same information).

Let's do another quick quiz about these outputs are!

```{r malsim_output_qs, exercise=TRUE}

quiz(
  question("What units are the age groups of outputs in?",
         answer("days", correct = TRUE),
         answer("weeks"),
         answer("months"),
         answer("years")),
...
```{r malsim_output_prev_vis, exercise=TRUE}
...
- Plot the light-microscopy detectable prevalence of 0.5-5 year olds through time.
