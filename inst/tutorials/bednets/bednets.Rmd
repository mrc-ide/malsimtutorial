...
Another widely used intervention that can be simulated using `malariasimulation` is the distribution of insecticidal bed nets (ITNs/LLINs). `malariasimulation` allows the users to tune a number of important components of both the bed nets to be distributed (e.g. efficacy, longevity) and the way in which they are distriubted (e.g. timing, coverage, and target population). In this tutorial, we will build on what we have learnt so far to learn how to simulate bed net distribution campaigns.

To simulate the distribution of bed nets, there are 4 steps:

1.  Load the base set of malariasimulation parameters (`get_parameters()`)
2.  Specify the bed net parameters (`set_bednets()`)
3.  Equilibrate the parameter set to an initial EIR (`set_equilibrium()`)
4.  Run the simulation (`run_simulation()`)
...
Let's take a look at a simple example of how to use `set_bednets()` to simulate bed net distributions when a single species of mosquito is simulated:
...
quiz(caption = "Parameters Qs",
     ...
     question("How many columns must the matrix inputs for dn0, rn, and rnm contain if we specify bed net events to be updated 5 times (i.e. we want parameters to be updated on 5 timesteps)?",
              answer("3"),
              answer("Depends on the number of mosquito species"),
              answer("1"),
...
# Plot the mosquito population through time:
simulation_output |>
  mutate(pfpr210 = n_detect_lm_730_3650 / n_730_3650) |>
...
Similar to `set_drugs()` in the treatment exercise, the `set_species()` function is used to load the mosquito species parameters of interest into our list of `malariasimulation` parameters. The `set_species()` function has three arguments:
...
matrix(c(0.533, 0.533, 0.33, 0.33), nrow = 2, byrow = TRUE)
