...
To simulate clinical treatment in malariasimulation, there are four main steps:
...
Using these four steps, we can simulate clinical treatment regimes that span from the very simple (single drug at a single coverage level through time) to the quite complex (multiple drugs with time-varying coverages). As we saw earlier, the first step when using malariasimulation is to load a set of parameters using `get_parameters()`:
...
```{r drug-params-quiz, exercise = TRUE}
...
clinical_treatment_simulation |>
  ggplot(aes(x = timestep, y = n_detect_lm_730_3650/n_730_3650)) +
...
2. Equilibrate the parameters to an initial EIR of 8 (refer the "basics" tutorial if you need to!)
...
6. Run both simulations for 5 years
7. Plot the *Pf*PR[2-10] for each simulation and compare the outputs 
